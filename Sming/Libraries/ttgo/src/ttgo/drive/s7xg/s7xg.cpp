...
const char* S7XG_Class::_gpsTypeArr[] = {"gps get_data raw", "gps get_data dms", "gps get_data dd"};
...
const char* S7XG_Class::_gpsFormatArr[] = {"gps set_format_uplink raw", "gps set_format_uplink ipso",
										   "gps set_format_uplink kiwi", "gps set_format_uplink utc_pos"};

const char* S7XG_Class::_gpsSystemArr[] = {"gps set_satellite_system gps", "gps set_satellite_system hybrid"};

const char* S7XG_Class::_loraPingpongComm[] = {"rf lora_rx_start 11223344556677889900",
											   "rf lora_tx_start 0 10 11223344556677889900", "rf lora_rx_stop",
											   "rf lora_tx_stop"};
	...
	while(_port->available()) {
		...
		case GPS_DATA_TYPE_RAW:
			if(0 == strncmp(data, RAW_POSITIONING_DONE, strlen(RAW_POSITIONING_DONE))) {
				sscanf(data, GPS_RAW_FORMAT, &year, &month, &day, &hour, &minute, &second, &lat, &latC, &lng, &lngC,
					   &timeStamp);
		...
		case GPS_DATA_TYPE_DD:
			if(0 == strncmp(data, DD_POSITIONING_DONE, strlen(DD_POSITIONING_DONE))) {
				sscanf(data, GPS_DD_FORMAT, &year, &month, &day, &hour, &minute, &second, &lat, &latC, &lng, &lngC,
					   &timeStamp);
				return GPS_Class(year, month, day, hour, minute, second, lat, lng);
			}
			break;
