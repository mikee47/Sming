...
{
public:
	GPS_Class(uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second, float lat,
			  float lng)
		: _year(year), _month(month), _day(day), _hour(hour), _minute(minute), _second(second), _lat(lat), _lng(lng)
	{
